/**
 * Tactical Country Intelligence Utility
 * Handles ISO conversion and visual flag identification.
 */
...
const ISO3_TO_ISO2: Record<string, string> = {
  ...
  'CYM': 'KY', 'CAF': 'CF', 'TCD': 'TD', 'CHL': 'CL', 'CHN': 'CN', 'CXR': 'CX', 'CCK': 'CC', 'COL': 'CO', 'COM': 'KM', 'COD': 'CD',
  'COG': 'CG', 'COK': 'CK', 'CRI': 'CR', 'HRV': 'HR', 'CUB': 'CU', 'CUW': 'CW', 'CYP': 'CY', 'CZE': 'CZ', 'CIV': 'CI', 'DNK': 'DK',
  ...
  'NGA': 'NG', 'NIU': 'NU', 'NFK': 'NF', 'MNP': 'MP', 'NOR': 'NO', 'OMN': 'OM', 'PAK': 'PK', 'PLW': 'PW', 'PSE': 'PS', 'PAN': 'PA',
  'PNG': 'PG', 'PRY': 'PY', 'PER': 'PE', 'PHL': 'PH', 'PCN': 'PN', 'POL': 'PL', 'PRT': 'PT', 'PRI': 'PR', 'QAT': 'QA', 'REU': 'RE',
  'ROU': 'RO', 'RUS': 'RU', 'RWA': 'RW', 'BLM': 'BL', 'SHN': 'SH', 'KNA': 'KN', 'LCA': 'LC', 'MAF': 'MF', 'SPM': 'PM', 'VCT': 'VC',
...
export const convertIso3To2 = (iso3: string): string => {
  if (!iso3) return '';
  const upper = iso3.toUpperCase();
  if (upper.length === 2) return upper;
  return ISO3_TO_ISO2[upper] || upper.substring(0, 2);
};

export const getFlagEmoji = (countryCode: string): string => {
  if (!countryCode) return '🌐';
  const code = convertIso3To2(countryCode);
